...
# ===== PR #27, Bugbot finding 1: geometry through l23's own wrappers =====
#
# prep_one_l23 used to seed fit_Bp/MCMC dimensions from rt_dict while
# fit_one/fit_with_LM/batch_fit passed 4-tuples with no ObsGeometry -- so no
# robust-backend (or fit_Bp) fit could run through this module's own
# convenience layer, and the synthetic observation Rrs stayed Gordon-only
# regardless of rt_dict['rt_backend']. These tests pin the fix: geometry is
# built in (geom_from_p; the L23 simulation's own 0-degree solar zenith by
# default, p.theta_s to override), threaded as the observation tuple's 5th
# element, and the synthetic observation is generated by the same forward
# model the fit uses.

def test_geom_from_p_policy():
    """geom_from_p: None for a legacy Gordon p; the L23 simulation's own
    geometry (theta_s=0, nadir) for a robust backend; p.theta_s/theta_v/
    dphi override when present (p_ntuple.gen accepts any extra field)."""
    from bing.rt.geometry import ObsGeometry

    p_gordon = standard.expb_pow(satellite='PACE', variable_Gordon=False)
    assert fit_l23.geom_from_p(p_gordon) is None

    p_robust = standard.expb_pow(satellite='PACE', variable_Gordon=False,
                                 rt_backend='robust_ztt')
    geom = fit_l23.geom_from_p(p_robust)
    assert isinstance(geom, ObsGeometry)
    assert geom.theta_s == fit_l23.L23_THETA_S == 0.0
    assert geom.theta_v == 0.0 and geom.dphi == 0.0

    p_over = standard.expb_pow(satellite='PACE', variable_Gordon=False,
                               rt_backend='robust_ztt', theta_s=30.)
    assert fit_l23.geom_from_p(p_over).theta_s == 30.

    # Explicit geometry attributes make even a Gordon p carry a geom
    # (harmless -- Gordon ignores it; the tuple contract calls it a no-op).
    p_g30 = standard.expb_pow(satellite='PACE', variable_Gordon=False,
                              theta_s=30.)
    assert fit_l23.geom_from_p(p_g30).theta_s == 30.


def test_prep_one_l23_robust_backend():
    """prep_one_l23 with a robust backend: exposes the geometry it used,
    and generates the synthetic observation with the robust forward model
    (measurably different from the Gordon-generated observation)."""
    from bing.rt.geometry import ObsGeometry

    idx = 170
    p_r = standard.expb_pow(satellite='PACE', add_noise=False,
                            variable_Gordon=False, rt_backend='robust_ztt')
    prep_r = fit_l23.prep_one_l23(p_r, idx)

    assert isinstance(prep_r['geom'], ObsGeometry)
    assert prep_r['geom'].theta_s == 0.0
    assert np.all(np.isfinite(prep_r['model_Rrs']))
    assert np.all(prep_r['model_Rrs'] > 0)

    # The observation is robust-generated, not Gordon-generated: ztt sits
    # a real few-to-several percent away from Gordon on L23 (M5 notebook).
    p_g = standard.expb_pow(satellite='PACE', add_noise=False,
                            variable_Gordon=False)
    prep_g = fit_l23.prep_one_l23(p_g, idx)
    assert prep_g['geom'] is None
    rel = np.abs((prep_r['model_Rrs'] - prep_g['model_Rrs'])
                 / prep_g['model_Rrs'])
    assert np.mean(rel) > 0.01, \
        f"robust observation looks Gordon-generated (mean rel {np.mean(rel):.2e})"

    # And it matches the shared raw-IOP adapter directly (same forward
    # model, same geometry, same true spectra -> same observation).
    odict = prep_r['odict']
    rt_dict = rt_defs.rt_dict_from_p(p_r)
    synth = evaluate.calc_Rrs_from_iops_robust(
        odict['a'], odict['bb'], odict['true_wave'], rt_dict,
        geom=prep_r['geom'])
    from bing.preproc import convert_to_satwave
    expected = convert_to_satwave(odict['true_wave'], np.squeeze(synth),
                                  prep_r['models'][0].wave)
    np.testing.assert_allclose(prep_r['model_Rrs'], expected, rtol=1e-6)


def test_fit_with_LM_robust_backend_end_to_end():
    """A robust-backend L23 LM fit through fit_with_LM converges, and its
    result is reconstructable (reconstruct_chisq_fits) and plottable
    (show_fits) with the geometry prep_one_l23 threaded -- the full loop
    the Bugbot findings showed was impossible before PR #27."""
    idx = 170
    p = standard.expb_pow(satellite='PACE', add_noise=False,
                          variable_Gordon=False, rt_backend='robust_ztt')
    ans, cov, models, prep, idx_out = fit_l23.fit_with_LM(p, idx)
    assert idx_out == idx
    ...
    # Reconstruction lands on the (noiseless, self-consistent) observation
    # to within the model-mismatch floor -- proof the fit really ran
    # through the robust forward model and converged.
    Chl = 10**ans[2] / 0.05582
    Rrs_fit, a_fit, bb_fit = evaluate.reconstruct_chisq_fits(
        models, ans, rt_dict, Chl=Chl, geom=geom)
    rel = np.abs((Rrs_fit - obs) / obs)
    assert np.median(rel) < 0.05, f"median rel err {np.median(rel):.2e}"

    # Visualization threads the same geometry through the LM branch.
    axes, model_Rrs = bing_plot.show_fits(
        models, ans, rt_dict, Chl, None, show=False, geom=geom,
        Rrs_true=dict(wave=models[0].wave, spec=obs,
                      var=prep['model_varRrs']))
    assert np.all(np.isfinite(model_Rrs))


def test_fit_with_LM_robust_fit_Bp():
    """fit_Bp through the L23 LM wrapper: p0 carries the B_p tail
    (prep_one_l23), the bounds gain the matching slot (fit_with_LM), and
    the fitted B_p stays inside the free-B_p prior range."""
    idx = 170
    p = standard.expb_pow(satellite='PACE', add_noise=False,
                          variable_Gordon=False, rt_backend='robust_ztt',
                          fit_Bp=True, Bp_value=0.014)
    ans, cov, models, prep, _ = fit_l23.fit_with_LM(p, idx)

    nmodel = models[0].nparam + models[1].nparam
    assert len(prep['p0']) == nmodel + 1, "p0 should carry the B_p tail"
    assert len(ans) == nmodel + 1
    assert rt_defs.BP_PRIOR_PMIN <= ans[-1] <= rt_defs.BP_PRIOR_PMAX

    # Reconstruction handles the tail (fit_func peels it) with geometry.
    rt_dict = rt_defs.rt_dict_from_p(p)
    Rrs_fit, _, _ = evaluate.reconstruct_chisq_fits(
        models, ans, rt_dict, Chl=10**ans[2]/0.05582, geom=prep['geom'])
    obs = prep['model_Rrs']
    rel = np.abs((Rrs_fit - obs) / obs)
    assert np.median(rel) < 0.05, f"median rel err {np.median(rel):.2e}"


def test_fit_one_robust_backend_mcmc():
    """A robust-backend L23 MCMC fit through fit_one: the 5-tuple carries
    prep_one_l23's geometry into inference.fit_one (a robust backend with
    a 4-tuple would raise at setup), the chains are finite, and the
    posterior reconstructs onto the robust-generated observation."""
    idx = 170
    # nsteps > the 7000-step default burn of thin_burn_chains, so the
    # standard reconstruction path applies unmodified (same choice as
    # test_raman_fitting_MCMC).
    p = standard.expb_pow(satellite='PACE', add_noise=False,
                          variable_Gordon=False, rt_backend='robust_ztt',
                          nsteps=10000, nburn=1000)
    np.random.seed(2026)
    chains, models, prep, idx_out, extras = fit_l23.fit_one(p, idx)

    assert idx_out == idx
    assert chains.shape == (10000, prep['pdict']['nwalkers'],
                            models[0].nparam + models[1].nparam)
    assert np.all(np.isfinite(chains))

    rt_dict = rt_defs.rt_dict_from_p(p)
    a_med, bb_med, a_lo, a_hi, bb_lo, bb_hi, Rrs_med, sigRs = \
        evaluate.reconstruct_from_chains(models, chains, rt_dict,
                                         geom=prep['geom'])
    obs = prep['model_Rrs']
    rel = np.abs((Rrs_med - obs) / obs)
    assert np.median(rel) < 0.1, f"median rel err {np.median(rel):.2e}"
    assert np.all(a_hi >= a_lo) and np.all(bb_hi >= bb_lo)


def test_batch_fit_robust_backend():
    """batch_fit threads one shared ObsGeometry through every observation
    tuple (multiprocessing pickling included) -- a small debug-mode robust
    batch runs end-to-end and writes the standard chain files."""
    p = standard.expb_pow(satellite='PACE', add_noise=False,
                          variable_Gordon=False, rt_backend='robust_ztt',
                          nsteps=100, nburn=10)
    with tempfile.TemporaryDirectory() as tmpdir:
        fit_l23.batch_fit(p, n_batch=2, n_cores=1, debug=True,
                          seed=42, out_dir=tmpdir)
        output_files = [f for f in os.listdir(tmpdir) if f.endswith('.npz')]
        assert len(output_files) == 4, "debug mode fits 4 spectra"
        loaded = np.load(os.path.join(tmpdir, output_files[0]),
                         allow_pickle=True)
        assert np.all(np.isfinite(loaded['chains']))
