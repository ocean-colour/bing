...
def entry_point():
    """Console-script entry point for the ``bing_fit_Rrs`` command.

    Parses command-line arguments and dispatches to :func:`main`.
    Wired up via ``[project.scripts]`` in ``pyproject.toml`` so that
    ``pip install`` exposes the ``bing_fit_Rrs`` executable.

    Inputs:
        None (reads ``sys.argv`` through :func:`parser`).
    Outputs:
        None (runs the fit and writes outputs as a side effect).
