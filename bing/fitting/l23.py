...
#: Solar zenith angle (degrees) of the L23 simulation this module fits.
#: load_one_l23/batch_fit load ``loisel23.load_ds(4, 0)`` -- Hydrolight
#: scenario X=4 at solar zenith Y=00 degrees (see ocpy.hydrolight.loisel23.
#: load_ds's docstring), nadir viewing. This is the *dataset's own,
#: documented* simulation geometry, not a guess -- see geom_from_p below.
L23_THETA_S = 0.0


def geom_from_p(p):
    """
    Build the fixed viewing/illumination geometry for an L23 fit from ``p``.

    PR #27 (Bugbot finding 1): robust backends require an ObsGeometry --
    ``validate_rt_dict`` raises without one -- but this module's own
    convenience wrappers (``fit_one``/``fit_with_LM``/``batch_fit``) used
    to build 4-tuples with no geometry, so no robust-backend fit could run
    through them. This helper is the built-in default the M4/M5 notebooks
    previously had to construct by hand.

    Geometry policy:

    - The Gordon backend with no geometry attributes on ``p`` returns
      ``None`` -- byte-for-byte legacy behavior (Gordon ignores geometry;
      its mean-cosine constants are baked in).
    - Otherwise an ObsGeometry is built from ``p.theta_s`` /
      ``p.theta_v`` / ``p.dphi`` when present (add them via
      ``standard.expb_pow(theta_s=30., ...)`` -- p_ntuple.gen accepts any
      extra field).
    - A robust backend with no ``p.theta_s`` falls back to
      ``L23_THETA_S`` (0 degrees) with nadir viewing. This is *not* a
      silent theta_s default in the CQ4 sense (claude_prompts/rob_rt.md,
      Q&A/Coding item 4): the observation being fit is generated by this
      very module from ``loisel23.load_ds(4, 0)``, whose simulation solar
      zenith is 00 degrees by construction -- the scene geometry is known
      dataset metadata, not an assumed angle.

    Parameters
    ----------
    p : namedtuple
        BING parameter tuple (bing.parameters.p_ntuple.gen). Consulted
        attributes (all optional): ``rt_backend``, ``theta_s``,
        ``theta_v``, ``dphi``.

    Returns
    -------
    bing.rt.geometry.ObsGeometry or None
        The fixed per-pixel geometry for this fit, or None for a Gordon
        fit with no geometry attributes set on ``p``.
    """
    has_geom_attrs = any(hasattr(p, key)
                         for key in ('theta_s', 'theta_v', 'dphi'))
    rt_backend = getattr(p, 'rt_backend', 'gordon')
    if rt_backend == 'gordon' and not has_geom_attrs:
        return None
    return ObsGeometry(theta_s=getattr(p, 'theta_s', L23_THETA_S),
                       theta_v=getattr(p, 'theta_v', 0.0),
                       dphi=getattr(p, 'dphi', 0.0))
...
def prep_one_l23(p, idx, chk:bool=False):
    ...
    dict
        ...
        - 'geom' : bing.rt.geometry.ObsGeometry or None
            Fixed per-pixel viewing/illumination geometry (geom_from_p);
            None for a legacy Gordon fit. The synthetic observation Rrs
            is generated with this geometry whenever rt_dict['rt_backend']
            selects a robust backend, and the fit wrappers thread it as
            the observation tuple's 5th element (PR #27).
    ...
    # Fixed viewing/illumination geometry (PR #27, Bugbot finding 1):
    # None for a legacy Gordon fit, an ObsGeometry otherwise -- see
    # geom_from_p's docstring for the policy (p.theta_s when present,
    # else the L23 simulation's own 0-degree solar zenith). Validated
    # eagerly, before any synthetic-Rrs work, so an illegal rt_dict/geom
    # combination (unknown backend, fit_Bp with Gordon, robust_hybrid
    # outside its trained wavelength range) raises here rather than
    # mid-generation -- the same fail-at-setup rule the fitters follow
    # (M2 task 3).
    geom = geom_from_p(p)
    ...
    if p.include_Raman:
        ...
        _Ed_pair = (_wv_Ed, _Ed_full)
    else:
        a_ex = None
        bb_ex = None
        _Ed_pair = None

    ## Calculate the synthetic observation Rrs from the dataset's true
    ## a/bb -- dispatched on the RT backend (PR #27, Bugbot finding 1):
    ## the observation must be generated with the *same* forward model the
    ## fit itself will use, so retrieval errors measure the inversion, not
    ## a forward-model mismatch. Before this fix the Gordon relation
    ## generated the observation regardless of rt_dict['rt_backend'].
    if rt_dict['rt_backend'] == 'gordon':
        ...
        if _Gb is not None:
            ...
            _in_bbp = None
        synth_Rrs = bing_rt.calc_Rrs(odict['a'], odict['bb'],
        ...
        ## Chl fluorescence (BING's own physics; the robust branch below
        ## composes fluorescence inside robust.rt.forward instead)
        if p.include_Chl_fl:
            ...
            synth_Rrs += bing_rt.calc_Rrs_fluorescence(
    ...
    model_Rrs = convert_to_satwave(l23_wave, synth_Rrs, model_wave)
    ...
    # Fixed per-pixel geometry for this fit (PR #27, Bugbot finding 1):
    # None for a legacy Gordon fit, the ObsGeometry the synthetic
    # observation above was generated with otherwise. fit_one/fit_with_LM/
    # batch_fit append it as the observation tuple's 5th element.
    ret_dict['geom'] = geom
...
def fit_one(p:namedtuple, idx:int,
    ...
    # Set the items -- the 5-tuple carries the fixed per-pixel geometry
    # prep_one_l23 built (PR #27, Bugbot finding 1). geom is None for a
    # legacy Gordon fit, which inference.fit_one documents as a strict
    # no-op; robust backends require it (validate_rt_dict raises at fit
    # setup otherwise).
    #p0 -= 1
    items = [(model_Rrs, model_varRrs, p0, idx, prep_dict['geom'])]
...
def fit_with_LM(p:namedtuple, idx:int, p0:np.ndarray=None):
    ...
    # Do it -- the 5-tuple carries the fixed per-pixel geometry
    # prep_one_l23 built (PR #27, Bugbot finding 1); None for a legacy
    # Gordon fit (a strict no-op per chisq_fit.fit's tuple contract).
    items = [(model_Rrs, model_varRrs, p0, idx, prep_dict['geom'])]
...
def batch_fit(p, n_batch:int=5, n_cores:int=15, debug:bool=False,
    ...
    # Fixed per-pixel geometry (PR #27, Bugbot finding 1): one geometry
    # for the whole batch -- every spectrum comes from the same L23
    # simulation (load_ds(4, 0)) and the same `p`, so prep_one_l23 builds
    # the identical ObsGeometry (or None, legacy Gordon) for each. An
    # ObsGeometry is a frozen dataclass, so sharing one instance across
    # the multiprocessing items is safe (it pickles by value).
    geom = prep_dict['geom']
    ...
    while i0 < len(all_idx):
        ...
        # Build the items -- 5-tuples; the trailing geom is a strict
        # no-op (None) for a Gordon fit and required for robust backends.
        items = []
        for idx in all_idx[i0:i1]:
            ss = all_idx.index(idx)
            item = (Rrs[ss], varRrs[ss], params[ss], idx, geom)
            items.append(item)
        ...
        for ss, item in enumerate(items):
            # Unpack (the trailing slot is the shared geom -- not needed
            # for output, the fit consumed it)
            iRrs, ivarRrs, iparams, idx = item[:4]
